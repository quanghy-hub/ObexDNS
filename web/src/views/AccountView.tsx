...
import { User, ShieldCheck, Trash2, UserPlus, Key, Edit2, Check, X } from "lucide-react";
...
export const AccountView: React.FC = () => {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newPassword.length < 8 || newPassword.length > 100 || !/(?=.*[a-zA-Z])(?=.*[0-9])/.test(newPassword)) {
  ...
  const handleCreateUser = async () => {
    ...
    if (newUserPassword.length < 8 || newUserPassword.length > 100 || !/(?=.*[a-zA-Z])(?=.*[0-9])/.test(newUserPassword)) {
  ...
  const handleDeleteMyAccount = async () => {
    ...
    try {
      ...
      if (res.ok) {
        window.location.href = "/"; // 刷新页面，触发重新登录
      } else {
        alert(await res.text());
      }
    } catch (e) { console.error(e); }
                ...
                <tbody>
                  {users.map(u => (
                    <tr key={u.id}>
                      ...
                      <td>
                        <Tag minimal intent={u.role === 'admin' ? Intent.DANGER : Intent.NONE}>
                          {u.role === 'admin' ? t("account.roleAdmin") : t("account.roleUser")}
                        </Tag>
                      </td>
        ...
        <div className="p-6 space-y-4">
          ...
          <FormGroup label={t("account.userRole")}>
            <HTMLSelect 
              ...
              options={[
                { label: t("account.roleUser"), value: "user" },
                { label: t("account.roleAdmin"), value: "admin" }
              ]}
            />
          </FormGroup>
